 ...
 */
public class PolarityLabelsRestaurantsHotels {

    String rootDirectory;
    PolarityLabelsRestaurantsHotels(int option, String trainFileName, String testFileName)
    {
    ...
    private void generateTestLabels(String dataFile, String labelFile ) {
        try {
            ...
            while ((line = read.readLine()) != null) {
                ...
                if (words[6].compareTo("negative") == 0) {          //words[6] for laptop set
                    write.println("-1");
                } else if (words[6].compareTo("positive") == 0) {
                    write.println("1");
                } else if (words[6].compareTo("neutral") == 0) {
                    write.println("0");
                } else if (words[6].compareTo("conflict") == 0) {
                    write.println("2");
                }
            }
            read.close();
