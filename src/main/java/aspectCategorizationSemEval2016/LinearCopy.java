...
public class LinearCopy {
     ...
     */
    public static void crossValidation(Problem prob, Parameter param, int nr_fold, double[] target, double[] withoutProbTarget)throws IOException {
        //PrintWriter write = new PrintWriter(new BufferedWriter(new FileWriter(System.getProperty("user.dir") + "\\dataset\\dataset_aspectCategorization\\predictedRestaurantsLabels.txt")));
        //BufferedReader testReader = new BufferedReader(new FileReader(new File(System.getProperty("user.dir") + "\\dataset\\dataset_aspectCategorization\\"+testFile)));
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            double[] probStore = new double[13];
            ...
            for (j = begin; j < end; j++) {
                ...
                double prediction = LinearCopy.predictProbability(submodel, prob.x[perm[j]], probStore);
                /*for(int p=0; p<13; p++){
                    withoutProbTarget[perm[j]][p] = probStore[p];
                }*/
            }
        }
    }
    ...
    public static double predict(Model model, Feature[] x) {
        ...
        //return predictProbability(model, x, dec_values);
    }
    ...
    public static double predictProbability(Model model, Feature[] x, double[] prob_estimates) throws IllegalArgumentException {
        ...
        int agreement = 0;
        max = model.label[(int)max];
        if(max == label){
            agreement = 1;
        }
        System.out.println("Max: "+max+", Label: "+label+", Agreement: "+agreement);
    ...
    public static double predictValues(Model model, Feature[] x, double[] dec_values) {
        ...
        double[] w = model.w;
        //System.out.println("LC: "+w.length);
        ...
        for (Feature lx : x) {
            ...
            if (idx <= n) {
                //System.out.println(idx+", "+n+", "+nr_w+", "+((idx - 1) * nr_w));
                for (int i = 0; i < nr_w; i++) {
        ...
        if (model.nr_class == 2) {
            ...
            for (int i = 1; i < model.nr_class; i++) {
                //System.out.println("******************* "+dec_values[i]);
                if (dec_values[i] > dec_values[dec_max_idx]) dec_max_idx = i;
            }
            //System.out.println(dec_max_idx);
