...
public class ABSAPolarityLaptops {
    ...
    ABSAPolarityLaptops(int option, String trainFileName, String testFileName, String pairFile) throws IOException {
        ...
        for (int i = 0; i < wordMap.size(); i++)    //Print the feature values in sorted order
        {
            ...
            while (iterator.hasNext() && counter<=5) {
                ...
                if(mentry.getValue().toString().compareToIgnoreCase("-0.0") != 0){
                    ...
                    URL oracle = new URL("http://maggie.lt.informatik.tu-darmstadt.de:10080/jobim/ws/api/turkishTrigram/jo/similar/" + mentry.getKey() + "?numberOfEntries=10&format=tsv");
